...
// === Constants (Legacy) ===
var GAN_LEGACY = ["\u7532", "\u4e59", "\u4e19", "\u4e01", "\u620a", "\u5df1", "\u5e9a", "\u8f9b", "\u58ec", "\u7678"];
var ZHI_LEGACY = ["\u5b50", "\u4e11", "\u5bc5", "\u536f", "\u8fb0", "\u5df3", "\u5348", "\u672a", "\u7533", "\u9149", "\u620c", "\u4ea5"];
...
function getTenGodLegacy(dayMaster, stem) {
    if (!stem) return '';
...
function calculateBaziLegacy(dateObj, gender, manualGZ = null, unknownHour = false) {
    ...
    const interactions = getInteractionsLegacy(processedPillars);
    ...
    const result = {
        ...
        gender: isMale ? '\u7537' : '\u5973',
        bodyStrength: calculateBodyStrengthLegacy(processedPillars)
    };

    if (typeof BaziProcessor !== 'undefined') {
        result.ctx = BaziProcessor.createContext(result);
    }
...
function calculateBodyStrengthLegacy(pillars) {
    const dm = pillars[2].gan;
...
function calculateYongXiJiLegacy(pillars, bsResult) {
...
function getInteractionsLegacy(pillars) {
    const stems = pillars.map(p => p.gan);
    const branches = pillars.map(p => p.zhi);
    const res = { stems: [], branches: [], judgments: [] };
    const ganHeMap = { '甲': '己', '己': '甲', '乙': '庚', '庚': '乙', '丙': '辛', '辛': '丙', '丁': '壬', '壬': '丁', '戊': '癸', '癸': '戊' };
...
// Expose functions to window for browser usage (Legacy names)
window.calculateBaziLegacy = calculateBaziLegacy;
window.calculateBodyStrengthLegacy = calculateBodyStrengthLegacy;
window.calculateYongXiJiLegacy = calculateYongXiJiLegacy;
window.findDateFromBazi = findDateFromBazi;
window.getInteractionsLegacy = getInteractionsLegacy;
...
window.GAN_LEGACY = GAN_LEGACY;
window.ZHI_LEGACY = ZHI_LEGACY;
window.GAN_WX_LEGACY = GAN_WX_LEGACY;
window.ZHI_WX_LEGACY = ZHI_WX_LEGACY;
