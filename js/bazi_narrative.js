...
const BaziNarrative = {
    ...
    /**
     * Generate a structured report object for AI consumption.
     * Splitting into: character, behavior, painPoint, solution, fengshui
     */
    generateStructuredReport: function (report) {
        if (!report || !report.bazi) return { error: "No Bazi Data" };
        ...
        // 1. Character (Inner Core + Day Master)
        let dmTrait = this.data.dm[dm] || "";
        ...
        let godTrait = (dayBranchGod && this.data.gods[dayBranchGod]) ? this.data.gods[dayBranchGod] : "";
        let character = `性格基调：${dmTrait}。内心深处：${godTrait}`;

        // 2. Behavior/Habits (Execution & Interaction)
        let behavior = "处世细节：在生活中是一个比较喜欢仪式感的人，喜欢被一些带有情绪价值的事物感动。虽然思考周密，但有时会因为过于细致而错过机会。";
        if (dayBranchGod === '七杀' || dayBranchGod === '比肩') behavior = "处世细节：执行力极强，做事不喜拖泥带水。早年间在人际关系上可能显得有些强势，认理不认人。但随着阅历增长，心态会逐渐平和。";

        // 3. Pain Point (Diagnosis Facts)
        let diagnosisType = 'wealth';
        let painPoint = "";
        const wBad = wealth?.matchTerms?.find(t => t.type === 'bad');
        const mBad = marriage?.matchTerms?.find(t => t.type === 'bad' || t.type === 'issue');

        if (wBad) {
            painPoint = `现实苦果（财富）：呈现“${wBad.title}”的状态。主要体现为${wBad.desc}`;
        } else if (mBad) {
            painPoint = `现实苦果（感情）：面临“${mBad.title}”的波动。具体为${mBad.desc}`;
        } else {
            painPoint = "现实苦果：目前虽然步调稳健，但陷入了某种“增长瓶颈”，缺乏破局的爆发力。";
        }

        // 4. Solution (Master Guidance)
        let solution = "大师建议：目前的现状并非死局。建议关注‘用神’带来的平衡。在环境上，留意是否有堆积杂物或光线不足的情况。心中气顺，则财路通。";

        return {
            character: this.stripBaziJargon(character),
            behavior: this.stripBaziJargon(behavior),
            painPoint: this.stripBaziJargon(painPoint),
            solution: this.stripBaziJargon(solution)
        };
    },
    ...
    generateScript: function (report) {
        const struct = this.generateStructuredReport(report);
        return [struct.character, struct.behavior, struct.painPoint, struct.solution].join("\n\n");
    },
    ...
    generateScriptLibrary: function (report) {
        ...
        if (report.expertData || (report.logicTargets && report.logicTargets.length > 0)) {
            const dtData = (report.expertData && report.expertData.decisionResult) ? report.expertData.decisionResult : {};
            let results = dtData.results || [];
            const trace = dtData.trace || [];

            // [SYNC FIX] Inject Tree Logic Targets if present
            if (report.logicTargets && Array.isArray(report.logicTargets)) {
                // Avoid duplication if they are already in results (unlikely with custom IDs)
                const existingIds = results.map(r => r.id);
                const bridgeResults = report.logicTargets
                    .filter(t => !existingIds.includes(t.id))
                    .map(t => ({
                        ...t,
                        _specificTrace: t._specificTrace || [] // Isolate from global dirty trace
                    }));
                results = [...results, ...bridgeResults];
            }
            ...
            results.forEach((res) => {
                if (window.logDebug) window.logDebug(`[Narrative Trace] Processing result: ${res.id}`, res);
                const resId = res.id;
                if (!resId) return;

                const idLower = resId.toLowerCase();
                const stepId_main = res.stepId_main ||
                    ((res._specificTrace && res._specificTrace.length > 0) ? res._specificTrace[0].id :
                        (trace && trace.length > 0 ? trace[0].id : ''));

                // 1. Identify Target Category
                let targetCat = res.category || 'wealth';
                const isFoundAcademic = idLower.includes('academic') || idLower.includes('elite') || idLower.includes('disciplined') ||
                    ...
                    idLower.includes('rebellion') || idLower.includes('achiever') || idLower.includes('dropout') ||
                    idLower.includes('broken') || idLower.includes('seal') || idLower.includes('unstable') ||
                    idLower.includes('cost') || idLower.includes('枭') || idLower.includes('偏印') || idLower.includes('yin') || idLower.includes('印') ||
                    idLower.includes('奇才') || idLower.includes('lucksync') || idLower.includes('timing') || (stepId_main && stepId_main.toLocaleLowerCase().includes('academic'));

                // [FORCE SYNC] Hard override for academic category to avoid wealth-prefix collision
                if (isFoundAcademic) {
                    targetCat = 'academic';
                }

                if (!res.category && targetCat !== 'academic') {
                    if (resId.startsWith('res_Career') || idLower.includes('career') || (stepId_main && stepId_main.includes('Career'))) {
                        targetCat = 'career';
                    } else if (resId.startsWith('res_Sex') || idLower.includes('sex') || (stepId_main && stepId_main.includes('Sex'))) {
                        targetCat = 'sexlife';
                    } else if (resId.startsWith('res_Child') || idLower.includes('child') || (stepId_main && stepId_main.includes('Children'))) {
                        targetCat = 'children';
                    } else if (resId.startsWith('res_Legal') || idLower.includes('legal') || (stepId_main && stepId_main.includes('Legal'))) {
                        targetCat = 'legal';
                    } else if (resId.startsWith('res_Rel') || resId.startsWith('res_Time') || resId.startsWith('res_Trait') ||
                        resId.startsWith('res_App') || idLower.includes('marriage') || (stepId_main && stepId_main.includes('Marriage'))) {
                        targetCat = 'marriage';
                ...
                let finalNarrative = res.narrative || res.desc || res.text || res.content || '';

                // [NARRATIVE SYNC] If academic but no content, try to find in specific trace or fallback
                if (!finalNarrative && targetCat === 'academic') {
                    if (res._specificTrace && res._specificTrace.length > 0) {
                        const lastNode = res._specificTrace[res._specificTrace.length - 1];
                        finalNarrative = lastNode.result || lastNode.content || '';
                    }
                }
                ...
                if (pattern) {
                    const patternContent = pattern.narrative || pattern.desc;
                    if (patternContent) {
                        finalNarrative = patternContent;
                        if (window.logDebug && targetCat === 'academic') window.logDebug(`[Academic Debug] Pattern matched and used: ${pattern.id}`);
                    }
                }

                if (window.logDebug && (targetCat === 'academic' || idLower.includes('logic'))) {
                    window.logDebug(`[Academic Debug] Processing [${targetCat}] ID:${res.id}`, {
                        keys: Object.keys(res),
                        textVal: res.text ? res.text.substring(0, 20) + "..." : "EMPTY",
                        descVal: res.desc ? res.desc.substring(0, 20) + "..." : "EMPTY",
                        finalVal: finalNarrative ? finalNarrative.substring(0, 20) + "..." : "EMPTY",
                        len: finalNarrative.length
                    });
                }

                // Intercept Academic for Merging
                if (targetCat === 'academic') {
                    const poolItem = {
                        id: resId,
                        title: res.title || (res.text ? res.text.split('】')[0].replace('【', '') : '学业点滴'),
                        text: finalNarrative,
                        tags: res.tags || (pattern ? pattern.tags : []),
                        path: pathStr,
                        isLuck: !!(resId.includes('Luck') || resId.includes('Year') || resId.includes('Sui') || resId.includes('Timing'))
                    };
                    academicPool.push(poolItem);
                    if (window.logDebug) window.logDebug(`[Academic Debug] Item pushed to pool. Current Pool Size: ${academicPool.length}`, poolItem);
                ...
                if (lib[targetCat]) {
                    ...
                    const resTitle = res.title || (res.text ? res.text.split('】')[0].replace('【', '') : '综合定论');
                    lib[targetCat].unshift({
                        title: `🔮 大师终极定论: ${resTitle}`,
            ...
            // --- 5. Academic Deep Fusion (Strict Logic Aggregation) ---
            if (academicPool && academicPool.length > 0) {
                if (window.logDebug) window.logDebug(`[Academic Fusion] Pool Size: ${academicPool.length}`, academicPool);
                // Use strict logic aggregation: Natal items first, then Luck/Year items.
                // No AI embellishment, just pure logical deduction texts.
                const natalItems = academicPool.filter(a => !a.isLuck);
                const luckItems = academicPool.filter(a => a.isLuck);

                // Intelligent Title Selection
                let bestTitle = "";
                // Priority 1: High-impact Natal findings (Dropout, Elite, Rebellious, etc.)
                const primaryNatal = natalItems.find(n => n.id.includes('Dropout') || n.id.includes('Elite') || n.id.includes('Rebellious') || n.id.includes('Achiever'));

                if (primaryNatal) {
                    bestTitle = (primaryNatal.title || "学业结论").replace(/[【】]/g, '');
                } else if (luckItems.length > 0) {
                    // Priority 2: Luck/Year influences
                    bestTitle = luckItems.map(l => (l.title || "学术运势").replace(/[【】]/g, '')).join(' + ');
                } else if (academicPool.length > 0) {
                    // Fallback
                    bestTitle = (academicPool[0].title || "学业分析").replace(/[【】]/g, '');
                }

                // Concatenate Texts Strictly
                const orderedPool = [...natalItems, ...luckItems];
                // Strip technical jargon but keep the original logic narrative intact
                const essay = orderedPool.map(item => {
                    const content = item.text || item.desc || "";
                    return this.stripBaziJargon(content).trim();
                }).filter(t => t.length > 0).join('\n\n');

                if (window.logDebug) window.logDebug(`[Academic Fusion] Generated Essay:`, { title: bestTitle, essayLength: essay.length });

                let combinedPaths = academicPool.map(a => a.path).filter(p => p).join(' + ');
                let allTags = [...new Set(academicPool.flatMap(a => a.tags))];

                lib.academic.unshift({
                    title: `🔮 大师终极定论: ${bestTitle}`,
                    content: `${essay}\n\n> [!NOTE]\n> **逻辑溯源**：${combinedPaths || '综合判定'}`,
                    pureContent: essay,
                    tags: allTags
                });
            } else {
                if (window.logDebug) window.logDebug(`[Academic Fusion] Pool is empty, skipping aggregation.`);
            }
        }
        ...
        if (lib.wealth.length === 0) {
            lib.wealth.push({
                title: "🔮 大师定论: 随缘求财",
                content: "命主目前的财富气场较为平稳，暂无极端的大起大落。建议以稳健经营为主，顺势而为，不可强求。建议在生活中注意理财习惯，厚积薄发。"
            });
        }
        ...
        if (lib.marriage.length === 0) {
            lib.marriage.push({
                title: "🔮 大师定论: 感情随缘",
                content: "婚姻宫磁场平稳，当前运势中并未显现剧烈的冲突或动荡。建议在相处中多沟通，保持平常心，自然能水到渠成。"
            });
        }
        ...
        if (lib.academic.length === 0) {
            lib.academic.push({
                title: "🔮 大师定论: 智慧增长",
                content: "学业/思维领域处于潜伏期。建议深耕专业领域，不急于立竿见影的成果。当前的积累将在未来运势交替时发挥关键作用。"
            });
        }
        ...
        if (lib.career.length === 0) {
            lib.career.push({
                title: "🔮 大师定论: 潜龙守正",
                content: "事业处于积蓄期。命主当前宜守不宜攻，在现有平台上打磨核心竞争力。官星气场尚未完全发动，静待流年契机。"
            });
        }
        ...
        if (lib.sexlife.length === 0) {
            lib.sexlife.push({
                title: "🔮 大师定论: 调和之道",
                content: "个人魅力磁场处于平稳期，生活节奏有度。建议保持良好的作息与心理弹性，情感交融顺其自然。"
            });
        }
        ...
        if (lib.children.length === 0) {
            lib.children.push({
                title: "🔮 大师定论: 后代福泽",
                content: "子女宫稳定，代表后代缘分平实。建议关注家庭互动，顺应孩子的天性发展，自有福报。"
            });
        }
        ...
        if (lib.legal.length === 0) {
            lib.legal.push({
                title: "⚖️ 大师定论: 洁身自好",
                content: "命局中官非磁场不显，目前属于安全期。只要日常遵纪守法、谨慎签署合同，基本无虞。注意人际中的口舌争执即可。"
            });
        }
     ...
     */
    /**
     * Generate Feng Shui environment analysis based on New Spatial Mapping (User Request).
     * Mapping:
     * - Year/Month Stems -> SW
     * - Year/Month Branches -> NW
     * - Day Branch -> N (Master Bedroom)
     * - Hour Stem -> SE
     * - Hour Branch -> NE (Bedroom)
     */
    generateFengShuiAnalysis(pillars) {
        ...
        // Helper to get Ten God
        const getTG = (targetStem) => window.getTenGod ? window.getTenGod(dm, targetStem) : null;

        // Helper to get Branch Main Qi Ten God
        const getBranchTG = (branch) => {
            if (window.HIDDEN_STEMS_MAP && window.HIDDEN_STEMS_MAP[branch]) {
                const mainQi = window.HIDDEN_STEMS_MAP[branch][0]; // Assume first is Main Qi
                return getTG(mainQi);
            }
            return null;
        };
        ...
        // Define Points of Interest - CORRECTED MAPPING (East/West Swapped)
        const points = [
            // Left Side (East Group) -> NOW SE/NE
            { label: '年干', char: pillars[0].gan, type: 'stem', group: 'SE', dirName: '东南方 (巽位)' },
            { label: '月干', char: pillars[1].gan, type: 'stem', group: 'SE', dirName: '东南方 (巽位)' },

            { label: '年支', char: pillars[0].zhi, type: 'branch', group: 'NE', dirName: '东北方 (艮位)' },
            { label: '月支', char: pillars[1].zhi, type: 'branch', group: 'NE', dirName: '东北方 (艮位)' },

            // Center (North)
            { label: '日支', char: pillars[2].zhi, type: 'branch', group: 'N', dirName: '正北方 (坎位·主卧)', isBedroom: true },

            // Right Side (West Group) -> NOW SW/NW
            { label: '时干', char: pillars[3].gan, type: 'stem', group: 'SW', dirName: '西南方 (坤位)' },
            { label: '时支', char: pillars[3].zhi, type: 'branch', group: 'NW', dirName: '西北方 (乾位·卧室)', isBedroom: true },
        ];

        // Group by Direction
        const grouped = {};
        points.forEach(p => {
            if (!grouped[p.group]) grouped[p.group] = { name: p.dirName, items: [] };
            grouped[p.group].items.push(p);
        });

        // 1. Seal Star Housing Conclusion (Existing Logic preserved)
        let zhengYinCount = 0;
        let pianYinCount = 0;
        let zhengYinToured = false;

        // Use simpler scan for Conclusion
        pillars.forEach((p, idx) => {
            const gGod = getTG(p.gan);
        ...
        // 2. Generate Directional Analysis
        Object.keys(grouped).forEach(key => {
            const group = grouped[key];
            let content = `**${group.name}**：\n`;

            // Special Combination Check: Mao-You (Rabbit-Rooster)
            const branchChars = group.items.filter(i => i.type === 'branch').map(i => i.char);
            if (branchChars.includes('卯') && branchChars.includes('酉')) {
                content += `  > 🚪 **特殊布局**：此方位蕴含“卯酉相冲”之象（金木交战，门户开合）。卯为门框，酉为玻璃/金属，组合而成 **推拉式玻璃门**、**大幅落地窗** 或 **带框移门**。\n`;
            }

            group.items.forEach(item => {
                const tg = item.type === 'stem' ? getTG(item.char) : getBranchTG(item.char);
                const mapping = item.type === 'stem' ? fs.stems[item.char] : fs.branches[item.char];

                if (mapping) {
                    content += `- [${item.label}·${item.char}] 映射为“${mapping}”`;
                    if (tg) content += ` (十神：${tg})`;
                    content += `。\n`;
                }

                // Ten God Object Logic (Apply to this specific item)
                if (tg) {
                    if (['正印', '偏印', '枭神'].includes(tg)) {
                        content += `  > 📦 **易有**：书籍、证书、长辈赠礼、佛像。`;
                        if (['偏印', '枭神'].includes(tg)) content += ` (或有翅膀/毛绒物品)`;
                        content += `\n`;
                    }
                    if (['正官', '七杀', '偏官'].includes(tg)) {
                        content += `  > ⚔️ **易有**：刀剑利器、奖杯、制服、健身器材。`;
                        if (['七杀', '偏官'].includes(tg)) content += `\n  > ⚠️ **警示**：若有散乱人形玩偶/旧照，易招小人（犯七杀）。**建议收纳进柜，不要外露**。`;
                        content += `\n`;
                    }
                    if (tg === '劫财') content += `  > 🎒 **易有**：他人遗留物、旧行李、借放物品。\n`;
                    if (tg === '食神') content += `  > 🍽️ **易有**：各种零食、好吃的、锅碗瓢盆。\n`;
                    if (tg === '伤官') content += `  > 🎨 **易有**：艺术品、乐器、创作工具、破损旧物。\n`;
                    if (['正财', '偏财'].includes(tg)) content += `  > 💰 **易有**：贵重物品、钱包、精美摆件。 (偏财多为流通之物)\n`;
                    if (tg === '比肩') content += `  > 🤝 **易有**：健身器材、成双成对的物品。\n`;
                }
            });

            res.push({ title: group.name, content: content });
        });

        res.push({
            title: "风水观测说明",
            content: "以上内容是根据您的命局空间分布反推的居住环境耦合点。方位映射遵循：年干月干->西南，年支月支->西北，日支->正北(主卧)，时干->东南，时支->东北。"
        });
