    ...
    function calculate(pillars, bsResult) {
        ...
        if (bothStrong && top1 && top2) {
            if (isClash(top1, top2)) {
                ...
                // [V2.1 核心逻辑] 针对木日主且金水过旺的情况（病重药轻）
                const isWoodDM = same === '木';
                const waterObj = profile.find(p => p.element === '水') || { score: 0 };
                const metalObj = profile.find(p => p.element === '金') || { score: 0 };
                const isBodyWeak = level.includes('身弱') || level.includes('中和');

                if (isWoodDM && isBodyWeak && waterObj.score > 12 && metalObj.score > 30) {
                    finalRes = {
                        mode: '病药',
                        yong: '土',
                        xi: '火',
                        ji: '金',
                        reason: '[核心逻辑] 木日主金水病重（金水总分过高），取土止水制金为主要方案'
                    };
                } else {
