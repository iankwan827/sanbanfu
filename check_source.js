...
const CHANG_SHENG = {
    ...
    '\u706b': { '\u5bc5': '\u957f\u751f', '\u536f': '\u6c91\u6d74', '\u8fb0': '\u51a0\u5e26', '\u5df3': '\u4e34\u5b98', '\u536f': '\u5e1d\u65fa', '\u672a': '\u8870', '\u7533': '\u75c5', '\u9149': '\u6b7b', '\u620c': '\u5e93', '\u4ea5': '\u7edd', '\u5b50': '\u80ce', '\u4e11': '\u517b' },
...
function getDynamicInteractions(pillars, targetIndices = null) {
    const res = getInteractions(pillars);
    if (!targetIndices || targetIndices.length === 0) return res;

    // Optional: Filter to only include interactions involving target indices
    // For now, returning all is safer to avoid missed logic.
    return res;
}
...
function calculateBodyStrength(pillars) {
    ...
    if (isSameParty(ZHI_WX[pillars[1].zhi])) {
        totalScore += weights.hourZhi * (isPillarVoid(1, pillars) ? 0.3 : 1.0);
    }
...
function calculateYongXiJi(pillars, bsResult) {
    ...
    const isClash = (w1, w2) => {
        const id1 = m[w1], id2 = m[w2];
        return (id1 + 2) % 5 === id2 || (id2 + 2) % 5 === id1;
    };
    const getBridge = (w1, w2) => {
        const id1 = m[w1], id2 = m[w2];
        return (id1 + 2) % 5 === id2 ? rev[(id1 + 1) % 5] : ((id2 + 2) % 5 === id1 ? rev[(id2 + 1) % 5] : null);
    };

    const dm = pillars[2].gan;
    const same = GAN_WX[dm], output = rev[(m[same] + 1) % 5], wealth = rev[(m[same] + 2) % 5], official = rev[(m[same] + 3) % 5], seal = rev[(m[same] + 4) % 5];
    const consumptionElements = [output, wealth, official];

    // --- Phase 1: Ingest Transmitted Tags ---
    // 直接从软件传参 (bsResult) 中获取五行 profile 和强弱标签
    // 软件已经算好了谁强谁弱，我们不需要在这层再算一遍分
    ...
    // 确定 Top 1 和 Top 2（基于软件传过来的分值排序，但决策看标签）
    const sorted = [...profile].sort((a, b) => b.score - a.score);
    if (sorted.length < 2) return { mode: '扶抑', yong: output, xi: wealth, ji: seal, reason: '基础平衡方案' };

    const top1 = sorted[0].element, top2 = sorted[1].element;

    // CASE SPECIFIC: 1987-9-12 男命
    // 软件传参数据：金 isStrong=true, 木 isStrong=false
    // 判定：不属于“两强均沾”，不触发修补层，保持默认。
    const bothStrong = getTag(top1) && getTag(top2);

    // --- Priority 1: Climate (调候) ---
    const mz = pillars[1].zhi;
    const waterScore = (profile.find(p => p.element === '水') || { score: 0 }).score;
    const fireScore = (profile.find(p => p.element === '火') || { score: 0 }).score;
    if (['巳', '午', '未'].includes(mz) && waterScore < 30) return { mode: '调候', yong: '水', xi: '金', ji: '火', reason: '夏生调候取水' };
    if (['亥', '子', '丑'].includes(mz) && fireScore < 30) return { mode: '调候', yong: '火', xi: '木', ji: '水', reason: '冬生调候取火' };

    // --- Priority 4: Initial Selection ---
    let finalRes = null;
    const level = bsResult.level;
    if (level.includes('身强')) {
        finalRes = { mode: '扶抑', yong: output, xi: wealth, ji: seal, reason: '身强取食伤财星' };
    } else if (level.includes('身弱')) {
        finalRes = { mode: '扶抑', yong: seal, xi: same, ji: wealth, reason: '身弱优先取印绶' };
    } else {
        finalRes = { mode: '扶抑', yong: output, xi: wealth, ji: seal, reason: '[平和]常规取泄耗' };
    }

    // --- Priority 5: Unified Repair Layer (Tag-Based) ---
    // 只有在软件认为“两强均旺”时，才视为平衡点偏离，需要修补
    if (bothStrong) {
        // [通关修补]: 两强相克 -> 取桥梁
        if (isClash(top1, top2)) {
            const bridge = getBridge(top1, top2);
            if (bridge) {
                const id1 = m[top1], id2 = m[top2];
                let destination = null;
                // Identify destination logic: (id1 + 2) % 5 === id2 means id1(Clash) -> Bridge -> id2(Target)
                if ((id1 + 2) % 5 === id2) destination = top2;
                else destination = top1;

                // [Safeguard]: If the destination clashes with Day Master (same) AND Day Master element is Weak
                // User Requirement: "Need to judge one more... e.g. Water is Weak"
                if ((m[destination] + 2) % 5 === m[same] && !getTag(same)) {
                    finalRes = {
                        mode: '通关转印',
                        yong: seal,
                        xi: same,
                        ji: wealth,
                        reason: `[修补-通关] 通关后五行(${destination})克日主且日主偏弱，改取印星(${seal})护身`
                    };
                } else {
                    const bIdx = m[bridge];
                    finalRes = {
                        ...
                        xi: rev[(bIdx + 4) % 5],
                        ji: rev[(bIdx + 3) % 5],
                        reason: `[修补-通关] ${top1}与${top2}均强且交战，取桥梁化解`
                    };
                }
            }
        }
        // [病药修补]: 两强相生且生身过重 -> 取财制印
            ...
            if ((product === same || product === seal) && getTag(same)) {
                finalRes = {
                    mode: '特殊病药',
                    yong: wealth,
                    xi: output,
                    ji: seal,
                    reason: `[修补-病药] ${top1}与${top2}均强且生身过重，取财星去病`
                };
            }
        }
    }

    // 修补 A: 五行缺失修补 (Missing Element Repair)
    const elements = ['木', '火', '土', '金', '水'];
    const missing = elements.find(el => (profile.find(p => p.element === el) || { score: 0 }).score === 0);
    if (missing) {
        const isBeneficial = (level.includes('身强') && consumptionElements.includes(missing)) ||
            (level.includes('身弱') && [same, seal].includes(missing));
        if (isBeneficial) {
            const mIdx = m[missing];
            finalRes = {
                mode: '补全',
                yong: missing,
                xi: rev[(mIdx + 4) % 5],
                ji: rev[(mIdx + 2) % 5],
                reason: `[修补-补全] 五行缺失且为喜用，取${missing}为用`
            };
        }
    }
...
function calculateAcademicStatus(data, earlyLucks = [], dynamicLuck = null, expertData = null) {
    try {
        ...
        if (expertData && expertData.decisionTree && expertData.decisionTree.results) {
            expertData.decisionTree.results.forEach(res => {
                if (!matchTerms.some(t => t.title === res.title)) {
                    matchTerms.push({
                        title: res.title,
                        desc: res.desc,
                        rate: 90
...
function calculateWealthStatus(data, currentLuck = null, expertData = null) {
    ...
    if (wealths.length > 0) {
        ...
        // [Engine Check] Use Expert Report for Combine Facts
        let engineCombineFact = null;
        if (expertData && expertData.summary) {
            const matches = expertData.summary.match(/([^，。]*?合[^，。]*)/g);
            if (matches) engineCombineFact = matches.join('，');
        }

        if (expertData ? engineCombineFact : wealths.some(w => w.isT && p.some(柱 => 柱.gan === ganHe[w.gan]))) {
            let desc = '这就是“财被合走”。你辛苦种的果子，眼看熟了，却容易被别人顺手摘走。这种象往往代表赚钱路上有绊子，财源被外界牵绊住了。你是不是常遇到快到手的钱被截胡？(对得上吗？)';
            if (engineCombineFact) desc += ` (引擎诊断依据：${engineCombineFact})`;
            matchTerms.push({ title: '财星被合', desc: desc, rate: 90 });
        }

        // 比劫夺财 (Section 3.2)
        const tWealths = wealths.filter(w => w.isT);
        const tBibei = bibei.filter(b => b.isT);
        if (tBibei.length > tWealths.length) {
            if (officers.some(o => o.isT)) {
                ...
                // [Engine Check] Use Expert Report for Clash/Rob Facts
                let engineCrashFact = null;
                if (expertData && expertData.summary) {
                    const matches = expertData.summary.match(/([^，。]*(冲|刑|克)[^，。]*)/g);
                    if (matches) engineCrashFact = matches.join('，');
                }

                let desc = '这个象比较明显，叫“比劫夺财”。说白了就是网兜破了个大洞，不管你往里装多少鱼，走两步就漏一条。你是不是常遇到借钱不还，或者莫名其妙就花冤枉钱？(这个漏财的洞你发现了吗？)';
                if (engineCrashFact) {
                    desc = `图一引擎侦测到【${engineCrashFact}】。这进一步验证了“漏财”的物理事实：根基受损，财库不稳。就像网兜不仅破了，还在被外力撕扯。你是不是觉得这钱正如流水般也留不住？(这下对明白了吧？)`;
                }
...
function calculateMarriageStatus(data, dynamicLuck = null, expertData = null) {
    try {
        ...
        // 1. Identify Spouse Star
        const targetGods = isMale ? ['正财', '偏财'] : ['正官', '七杀'];
        const spouseStars = [];
        let hasZheng = false;
        let hasPian = false;

        // Scan Pillars for Spouse Stars
        p.forEach((pillar, i) => {
            // Stem
            const gGod = getTenGod(pillar.gan);
            if (targetGods.includes(gGod)) {
                // Score: Rooted? Season? (Simplified Score)
                let score = 10;
                spouseStars.push({ i, pos: 'gan', god: gGod, val: pillar.gan, wx: GAN_WX[pillar.gan], score });
            ...
            hidden.forEach((h, hIdx) => {
                const hGod = getTenGod(h);
                if (targetGods.includes(hGod)) {
                    let score = (hIdx === 0) ? 8 : 4; // Main Qi is stronger
                    spouseStars.push({ i, pos: 'zhi', god: hGod, val: h, wx: GAN_WX[h], score, branch: pillar.zhi });
                    if (hGod === targetGods[0]) hasZheng = true;
                    if (hGod === targetGods[1]) hasPian = true;
                }
            });
            // Interactions (Chong/Xing) for Palace
            const interactions = data.interactions || (data.bazi && data.bazi.interactions);
            ...
            // Tomb/Grave Check for Spouse Star
            const spouseWx = targetGods[0].includes('财') ? (isMale ? GAN_WX[dm] : null) : (!isMale ? GAN_WX[dm] : null); // Roughly
            // A better way: get WX of the spouse star itself
            const sWx = spouseStars.length > 0 ? spouseStars[0].wx : null;
            let isStarInTomb = false;
        }); // End of p.forEach for spouseStars identification
        ...
        let isStarInTomb = false;
        if (sWx) {
            const tombZhi = { '木': '未', '火': '戌', '金': '丑', '水': '辰', '土': '辰' }[sWx];
            isStarInTomb = p.some(pillar => pillar.zhi === tombZhi);
        }

        // Specific Flags for Happy/Divorce/Affair
        const isStrongDM = acS.includes('强');
        // Assuming dzGod is defined elsewhere, e.g., dzGod = getTenGod(dz);
        const dzGod = getTenGod(HIDDEN_STEMS_MAP[dz]?.[0] || dz); // Define dzGod here if not already
        ...
        const isYangRen = p.some(pillar => pillar.zhi === YANG_REN_MAP[dm]);
        ...
        const dzMainGod = dzHidden[0] ? getTenGod(dzHidden[0]) : '';
        ...
        if (hasStarPalaceHarm) {
            ...
            if (!dayCombined && !isHarmRescued) {
                matchTerms.push({ type: 'issue', title: '星宫互穿 (婚姻危机)', desc: '婚姻宫与夫/妻星发生了穿害（' + dz + harmingBranch + '相穿），且原局无救应。说明家里容不下配偶，易导致离婚或配偶身体大问题。', rate: 90 });
            }
        }
        ...
        if (allHidden) lateTerms.push(isMale ? '妻星藏而不透' : '夫星藏而不透');
        ...
        if (!isMale) {
            ...
            const allHidden = spouseStars.every(s => s.pos === 'zhi');
            if (allHidden) lateTerms.push('妻星藏而不透');
        }
        ...
        const allStems = p.map(pi => pi.gan);
        let isSpouseStemCombined = false;

        spouseStars.forEach(s => {
            if (s.pos === 'gan') {
                const starStem = p[s.i].gan;
                const targetHe = GAN_HE[starStem];
                // Check if targetHe exists in OTHER columns (not self)
                // Actually, standard He is usually adjacent (Year-Month, Month-Day, Day-Hour).
                // But for "Affair", any presence of the combination partner implies "Temptation".
                // Let's check if 'targetHe' exists anywhere else in chart (ignoring Day Master combination if DM is the target? No, DM combining with Spouse Star is GOOD/Happy).
                // User text: "Combined with Year/Month, or Hour".
                // So if Spouse Star combines with DM, it's legitimate. If combines with OTHERS (Bi Jie/Cai/etc), it's Affair.
                // WE MUST EXCLUDE DAY MASTER COMBINATION.
                // Iterate all columns: if col.gan == targetHe AND col.index != 2 (Day God)
                // Wait, if Spouse Star IS Day Stem? Impossible (Day Stem is DM).
                // If Spouse Star is Month Stem, and combines with Year Stem -> Affair.
                // If Spouse Star is Month Stem, and combines with Day Stem -> Happy/Bond.
                // So we check: Does starStem combine with any Non-DM Stem?
                const isCombinedWithOther = allStems.some((stem, idx) => {
                    return stem === targetHe && idx !== 2; // Index 2 is Day Master
                });
                if (isCombinedWithOther) isSpouseStemCombined = true;
            }
        });
        ...
        const isYangRenStrong = isYangRen; // Assume strong if present for risk check
        ...
        const treeContext = {
            ...
            isSpouseStemCombined, // Added for Stem Combine Check
            ...
            hasLiuHe, // For Affair Logic
            hasAnHe,   // For Affair Logic
